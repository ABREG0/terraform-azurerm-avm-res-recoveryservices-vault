> Note
> This example pins both `azurerm` and `azapi` providers to `var.subscription_id` in `provider.tf`.
> This prevents Azure CLI context drift from producing cross-subscription plans.
